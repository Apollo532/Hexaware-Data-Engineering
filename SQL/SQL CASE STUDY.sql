...
('B', '2021-01-09');
 ...
 --Q1 Amount spent by each customer
...
-- Q4 Most Purchased Item and Quantity
...
--Q5 Most popular item by Customer
...
HAVING COUNT(*) = (
        ...
        FROM (SELECT COUNT(*) AS count 
			  FROM sales s2 
              WHERE s2.customer_id = s.customer_id 
              GROUP BY s2.product_id) AS counts
			      );

--Q6 First Item Purchased After becoming a member

SELECT TOP 3 s.customer_id, m.product_name
FROM sales s
JOIN menu m ON s.product_id = m.product_id
...
SELECT TOP 1 s.customer_id, m.product_name
FROM sales s
JOIN menu m ON s.product_id = m.product_id
JOIN members mem ON s.customer_id = mem.customer_id
WHERE s.order_date < mem.join_date
...
--Q8 Total Items and Spend before becoming member
...
--Q9 Points earned by each Customer
...
--Q10 Points earned by A & B in first week of membership, before the end of January
SELECT s.customer_id,
    SUM(
        CASE 
